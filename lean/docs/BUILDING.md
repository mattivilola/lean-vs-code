...
For a new minor release (`x.y.0`), run and publish the full paired benchmark against original Code-OSS at the same upstream revision, and add the result to the version history. A routine patch release does not require that benchmark: keep the last minor release's figures labeled with the exact measured version. Run targeted performance checks for a suspected regression or a performance-sensitive patch. Every release still needs the functional, extension, signing, notarization, packaging, and update checks applicable to its changes.
