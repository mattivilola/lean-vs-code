...
export function createNativeAboutDialogDetails(productService: IProductService, osProps: IOSProperties): { title: string; details: string; detailsToCopy: string } {
	...
	const getDetails = (useAgo: boolean): string => {
		if (productService.leanReleaseVersion) {
			return localize({ key: 'leanAboutDetail', comment: ['Electron, Chromium, Node.js and V8 are product names that need no translation'] },
				"Lean release: {0}\nCode-OSS API: {1}\nCommit: {2}\nDate: {3}\nElectron: {4}\nChromium: {5}\nNode.js: {6}\nV8: {7}\nOS: {8}",
				productService.leanReleaseVersion,
				version,
